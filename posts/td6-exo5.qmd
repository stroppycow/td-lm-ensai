...
Le but de l’exercice est de modéliser la variable $\texttt{imapir}$.
...
$\texttt{imapir}$ et les autres variables du jeu de données.
...
\end{align*}
$$
On estime les rapports en prennant une des modalités comme référence (par défaut, {{< iconify logos:r-lang >}} choisit la première dans c'est à dire la modalité 1):
...
data.frame(
  `Modèle`=c("Avec impair et ses", "Avec ses uniquement", "Avec life uniquement", "Modèle nul"),
