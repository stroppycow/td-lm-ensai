...
#| autorun: true
modele_q3 <- glm(
  formula = (as.numeric(fault)-1) ~ age+brand,
...
modele_q3_bis <- glm(
  formula = (as.numeric(fault)-1) ~ age +brand,
  family = binomial(link = 'logit'),
  contrasts =  list(brand = contr.treatment(levels(data_break$brand), base =2)),
  data = data_break
)
...
modele_q3_ter <- glm(
  formula = (as.numeric(fault)-1) ~ age + brand,
  family = binomial(link='logit'),
  contrasts =  list(brand = contr.sum(levels(data_break$brand))),
  data = data_break
)
...
\textrm{logit}(\mathbb{P}(\texttt{fault}=1 | \texttt{age}&=x_{\texttt{age}},\texttt{brand}=2))=\gamma_0+\beta_1x_\texttt{age} - \gamma_{21} -\gamma_{22}\\
\end{align*}
$$
...
#| autorun: true
anova(
  glm(formula = (as.numeric(fault)-1) ~ 1, family = binomial(),  data = data_break),
  modele_q3,
  test = 'Chisq'
)
```
...
#| autorun: true
modele_q5 <- glm(
  formula = (as.numeric(fault)-1) ~ age,
  family = binomial(link = 'logit'),
  data = data_break
)
summary(modele_q5)
...
On inclut un terme quadratique en `age`. Ce nouveau modèle s'écrit :
...
$$
\textrm{logit}(p(x_{\texttt{age}}))=\beta_0+\beta_1x_\texttt{age}+\beta_2x_\texttt{age}^2$$

Afin de prendre en compte l'âge au carré dans le modèle, on peut ajouter
une nouvelle variable dans le `data.frame` contenu dans le paramètre
`data` de la fonction `glm` et ajouter le nom de cette nouvelle variable
dans le paramètre `formula`. Ou alors, on peut utiliser la fonction `I`
dans le paramètre `formula` pour définir une nouvelle variable à partir
...
Les coefficients pour `age` et `age^2` sont significatifs pour un risque
de première espèce de 5 %.

On peut, par ailleurs, faire un test du rapport de vraisemblance pour
analyser la significativité globale du modèle :
...
On rejette l'hypothèse nulle : le modèle est significatif.

On peut aussi regarder si ce modèle est meilleur que le modèle de la
question 5 par exemple avec l'AIC.

::: {.callout-note mode="simple"}
### Rappel

Le critère d'information d'Akaike s'écrit comme suit :
$$\textrm{AIC} = 2d- 2\ln{L_n}$$ avec $d$ le nombre de paramètre dans le
modèle et $\ln{L_n}$ la vraisemblance du modèle.

Si l'on considère un ensemble de modèles candidats, le modèle choisi par
ce critère est celui qui aura la plus faible valeur d'AIC. Ce critère
repose donc sur un compromis entre la qualité de l'ajustement ($L_n$) et
la complexité du modèle ($d$), en pénalisant les modèles ayant un grand
nombre de paramètres
:::

Dans le modèle de la question 5, il vaut 49.5978 alors qu'ici
il vaut 37.2792. On retient donc plutôt ce modèle.
...
#| autorun: true
#| fig-width: 8
vecteur_ages <- seq(from = 0, to = 16, by = 0.25)
probas_predites_q5 <- predict(
  object = modele_q5,
  newdata = data.frame(age = vecteur_ages),
  type = "response"
)
probas_predites_q6 <- predict(
  object = modele_q6,
  newdata = data.frame(age = vecteur_ages),
  type = "response"
)
...
plot(
  data_break$age, as.numeric(data_break$fault) - 1,
  col = "#949FA6", pch = 16, 
  xlab = "Âge du véhicule",
  ylab = "Probabilité que la voiture ait connu une panne",
  main = "Probabilités empiriques et probabilités prédites"
)

# Ajout des courbes prédites
lines(
  vecteur_ages, probas_predites_q5, 
  col = "#F24B4B", lwd = 2, lty = 1
)
lines(
  vecteur_ages, probas_predites_q6, 
  col = "#68A8B0", lwd = 2, lty = 1
)

# Ajout de la légende
legend(
  "right", 
  legend = c("Données observées", "Probabilité prédite Q5", "Probabilité prédite Q6"),
  col = c("#949FA6", "#F24B4B", "#68A8B0"),
  pch = c(16, NA, NA),
  lty = c(NA, 1, 1),
  lwd = c(NA, 2, 2),
  bty = "o",
...
ggplot2::ggplot() +
  ggplot2::geom_point(
    mapping = ggplot2::aes(x = age,y = (as.numeric(fault)-1) , color = "obs"),
    data = data_break
  ) +
  ggplot2::geom_line(
    mapping = ggplot2::aes(x = age,y = p_q5, color = "pred5"),
    data = data.frame(age=vecteur_ages, p_q5=probas_predites_q5)
  ) +
  ggplot2::geom_line(
    mapping = ggplot2::aes(x = age,y = p_q6, color = "pred6"),
    data = data.frame(age=vecteur_ages, p_q6 = probas_predites_q6)
  ) +
  ggplot2::labs(
    title = "Probabilités empiriques et probabilités prédites",
    x = "Âge du véhicule",
    y = "Probabilité que la voiture ait connu une panne"
  ) +
  ggplot2::scale_color_manual(
    values=c("pred5"="#F24B4B", "pred6"="#68A8B0", "obs"="#949FA6"),
    name="",
    labels= c("pred5"="Probabilité prédite Q5","pred6"="Probabilité prédite Q6", "obs"="Données observées")
) +
  ggplot2::theme_light() +
  ggplot2::theme(legend.position = "bottom")
```
...
Le modèle quadratique semble graphiquement plus satisfaisant même si
l'ajustement n'est pas parfait.

## Question 7

On ajoute la variable `brand` par rapport au modèle précédent. Le modèle
estimé est désormais :
...
#| autorun: true
modele_q7 <- glm(
  formula = (as.numeric(fault) - 1) ~ age + I(age**2) + brand,
  family = binomial(link='logit'),
  data = data_break
)
summary(modele_q7)
...
#| envir: baser
#| autorun: true
exp(confint.default(modele_q7, parm = "brand2"))
```
...
$$ D_{H_0} - D_{H_1} \underset{H_0}{\sim} \chi^2(2)$$
On ne rejette pas l'hypothèse nulle ici car on a :
...
#| envir: baser
#| autorun: true
modele_q6$deviance-modele_q7$deviance
qchisq(p = 0.95, df = 2)
```
...
#| envir: baser
#| autorun: true
anova(modele_q6, modele_q7, test = 'Chisq')
```
