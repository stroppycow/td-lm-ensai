...
On considère les données `break`. 
Ce jeu de données, de taille $n = 33$, comporte trois variables ayant trait
à l’état d’une automobile :

- Une variable $\texttt{fault}$ qui vaut 1 si la voiture concernée a connu une panne, 0 sinon ;
- Une variable $\texttt{age}$ qui donne l’âge de la voiture ;
- Une variable $\texttt{brand}$  qui donne la marque de la voiture.

Le but de l’exercice est de modéliser la variable $\texttt{fault}$.

## Question 1

Importer les données sous {{< iconify logos:r-lang >}} et recoder la classe
de chaque variable si nécessaire.

::: {.content-visible when-meta="is_answer_print_1"}

::: answer

Importons le fichier de données `break.csv` en {{< iconify logos:r-lang >}} sous forme de
`data.frame`.
...
La variable $\texttt{age}$ est quantitative (l'âge de la voiture) alors que
...
Observer graphiquement le lien éventuel entre $\texttt{fault}$ et $\texttt{age}$ d’une part,
et entre $\texttt{fault}$ et $\texttt{brand}$ d’autre part.
...
On souhaite mettre en oeuvre un modèle de régression logistique expliquant la probabilité
d’avoir une panne en fonction de l’âge de la voiture et de sa marque. Lancer
cette modélisation sous {{< iconify logos:r-lang >}} et écrire la formule mathématique du modèle obtenu. On
précisera en particulier le modèle estimé spécifique aux voitures de la marque 0, puis
...
indicatrice de la présence de cette modalité. Comme la variable $\texttt{brand}$
...
Si on détaille l'expression du modèle par marque comme demandée dans
l'enoncé, on a:
...
On pourait, par exemple, choisir la deuxième modalité (i.e.
...
associés aux indicatrices pour la variable $\texttt{brand}$ :
...
aussi des modalités de la variable $\texttt{brand}$. La variable `brand1`
correspond à la première variable de contraste pour $\texttt{brand}$ et non à une
...
Analyser la qualité globale du modèle.
...
Recommencer la modélisation en incluant seulement la variable $\texttt{age}$ dans le modèle.
Est-ce mieux ?
...
On construit un nouveau modèle avec seulement un terme linéaire en $\texttt{age}$
...
Ce modèle ne semble pas beaucoup mieux. La variable $\texttt{age}$ n'est pas
significative dans ce modèle pour un seuil d'erreur de 5 %. Le fait que
la variable $\texttt{age}$ ne soit pas significative dans le modèle est
...
semble suggérer que les pannes ont lieu en début de vie du véhicule 
("défauts de jeunesse" ou de "rodage") et en fin de vie (pannes d’usure).
Ce comportement de la probabilité de pannes en forme de parabole nous incite
à essayer d’inclure un terme quadratique en la variable $\texttt{age}$.
Effectuer cet ajout dans le modèle en incluant également la variable $\texttt{brand}$ et
analyser les résultats. Le modèle est-il significatif ?
...
On rejette l'hypothèse nulle de globalité de tous les coefficients en
dehors de la constante au seuil 5 % car la $\textit{p-value}$ est inférieure à 0,05.
Le modèle est significatif cette fois-ci.

::: {.callout-note}
### Intérêt du terme quadratique
...
age <- seq(from = 0, to = 15, by = 0.05)
fake_data <- data.frame(
  age   = rep(age, 3),
  brand = rep(as.factor(c("0", "1", "2")), each = length(age))
)

## Prédictions
fake_data$p_hat <- predict(modele_q6, newdata = fake_data, type = "response")
...
plot(
  ...
  type = "n",
  xlab = "Âge",
  ylab = "Probabilité estimée d'un véhicule de tomber en panne"
)

# Définition des couleurs
cols <- c("0" = "black", "1" = "red", "2" = "blue")


# Ajout des autres marques
for (b in c("0", "1", "2")) {
  dat <- fake_data[fake_data$brand == b, ]
  lines(dat$age, dat$p_hat, col = cols[b], lwd = 2)
}
...
ggplot2::ggplot(
  data = fake_data,
  mapping = ggplot2::aes(x = age, col = brand, y = p_hat)
) +
  ggplot2::geom_line(linewidth = 1) +  # Ajouter la courbe
  ggplot2::labs(
    x = "Âge", 
    y = "Probabilité estimée d'un véhicule de tomber en panne"
  ) +
  ggplot2::theme_light()
```
:::
...
Que vaut l’odds ratio associé à la marque "2" par rapport à la marque "0" de la
variable $\texttt{brand}$ ? Interpréter cette valeur et donner un intervalle de
confiance à 95 % autour de cette estimation. L’inclusion de la variable $\texttt{brand}$
dans le modèle est-il pertinent ?
...
Pour tester, globalement la significativité de $\texttt{brand}$, on peut faire un
...
La variable $\texttt{brand}$ ne semble pas être significative.
