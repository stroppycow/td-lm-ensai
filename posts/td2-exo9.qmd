...
loi $F(1, n-p)$. Or par définition d'une loi de Fisher,
...
pour tout $x$ (c'est la convergence en loi). La fonction
$F_{\chi^{2}(1)}(.)$ étant continue et croissante, cette convergence
...
sélectionner le mauvais modèle (celui incluant $X^{(p)}$) demeure positive même lorsque $n \rightarrow \infty$. Seul le BIC assure la sélection du bon modèle lorsque $n \rightarrow \infty$, ce qui en fait le critère préférable sous cet aspect.
