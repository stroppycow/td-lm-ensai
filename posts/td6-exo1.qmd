...
Le jeu de donnée `chdage.txt`, contient les données
de 100 patients âgés de 20 à 69 ans (variable $\texttt{age}$), présentant pour certains une maladie coronarienne (variable $\texttt{chd}$ valant $\texttt{Yes}$ ou $\texttt{No}$).

## Question 1

Importer ces données sous {{< iconify logos:r-lang >}} sous la forme d’un `data.frame`. Observer le contenu de chaque variable et transformer leur `class` si nécessaire.

::: {.content-visible when-meta="is_answer_print_1"}

::: answer

On charge les données du fichier `chdage.txt` en {{< iconify logos:r-lang >}} sous forme de
`data.frame`.
...
Proposer une ou des visualisation(s) graphique(s) permettant d’analyser le lien éventuel
...
légèrement asymétriques tout de même pour les individus malade). De
plus, l'égalité des variances semble plausible au vu des boxplots.
...
En s’appuyant sur cette loi, donner la vraisemblance des observations $(y_1, \dots , y_n)$
en fonction de $\left(p(x_1), \dots , p(x_n)\right)$ où $x_i$ désigne l’âge de l’individu $i$ et 
...
En guise de première estimation de $p(x)$, on met en oeuvre la démarche suivante :

a. Utiliser les 8 groupes d’âge proposés via la variable $\texttt{agegrp}$ du jeu de données,
ce qui forme 8 groupes d’individus associés à ces classes. Calculer 
$\overline{x}_1, \dots , \overline{x}_8$ le milieu de chaque classe d’âge.
b. Calculer les proportions de $\texttt{chd} = \texttt{Yes}$ dans chaque groupe, notées 
...
Afin d’analyser la qualité de l’estimation précédente, transformer la variable 
$\texttt{chd}$ en variable numérique prenant les deux valeurs 0 et 1, et 
représenter sur un même graphique le nuage de points de 
$\texttt{age}$ et $\texttt{chd}$ (recodé) et les proportions estimées dans
...
Dans cette question et la précédente, on modélise en fait la loi de
d'être atteint par une maladie coronarienne conditionnellement à la
classe d'âge des individus (i.e. la variable aléatoire
$Y|(\texttt{agegrp}=x)$).
...
On décide de modéliser $p(x)$ à l’aide d’un modèle de régression logistique de
paramètre $\beta = (\beta_0, \beta_1) \in \mathbb{R}^2$ 

Quelle hypothèse cela signifie-t-il sur l’expression de $p(x)$ ? 

Cela est-il compatible avec le graphique précédent ? 

Quelle(s) autre(s) alternative(s) de modélisation pourrait-on suggérer ?
...
Il peut être utile de définrir en {{< iconify logos:r-lang >}}, les deux fonctions vectorielles
suivantes :
...
Ecrire la log-vraisemblance du modèle logistique en fonction de $\beta$ et en déduire le
système que doit résoudre le maximum de vraisemblance $\hat{\beta}$.

Peut-on résoudre ce système analytiquement ?
...
Calculer l’estimateur du maximum de vraisemblance via la fonction `glm`.
...
On peut utiliser la fonction `glm` de {{< iconify logos:r-lang >}} pour l'approximation numérique.
C'est ce qu'on doit faire en pratique. Les arguments sont semblables à
la fonction `lm`. Il faut faire attention à bien paramétrer l'argument
`family` sans quoi on estimerait un modèle de régression linéaire
simple.
...
Rappeler la définition théorique du rapport de cotes (odds ratio) d’avoir une maladie
coronarienne entre un individu d’âge $x_1$ et un individu d’âge $x_2$. Que vaut-il pour le
modèle précédent lorsque les 2 individus ont 10 ans d’écart (c’est à dire $x_1 = x_2+10$) ?

::: {.content-visible when-meta="is_answer_print_11"}

::: answer

La cote (l'odds) pour un individu avec des caractéristiques $x$ est le
ratio entre la probabilité que l'événement (ici avoir un maladie
coronarienne) se produise et la probabilité qu'il ne se produise pas.

$$\textrm{odds}(x)=\frac{p(x)}{1-p(x)}$$

L'odds-ratio (OR) d'avoir une maladie coronarienne entre un individu
d'âge $x_1$ et un autre individu d'âge $x_2$ est le rapport :
...
est environ multipliée par 3 tous les 10 ans toutes choses égales par
ailleurs.
...
On s’intéresse à présent au rapport de probabilités (et non de cotes) d’avoir une
maladie coronarienne entre un individu d’âge $x_1$ et un individu d’âge $x_2$. Que vaut
ce rapport pour le modèle précédent lorsque les 2 individus ont 10 ans d’écart (c’est
à dire $x_1 = x_2 + 10$) ? On pourra représenter ce rapport en fonction de $x_2$, pour $x_2$ prenant des valeurs de 20 à 70 ans.
...
Contrairement au rapport de cote, le rapport de probabilité dépend de
l'âge $x_2$.
...
On rappelle que sous "de bonnes conditions", l’estimateur du maximum de vraisemblance $\beta$ dans un modèle de régression logistique portant sur $p$ variables explicatives et 
$n$ individus vérifie la convergence en loi suivante, lorsque $n \to \infty$,
...
où $\boldsymbol{X}$ est la matrice de design et $W_{\beta}$ est la matrice diagonale
...
Justifier qu’une approximation asymptotique de la loi de $(\hat{\beta}-\beta)$ est $\mathcal{N}\left(0_p, J_n^{-1}(\beta)\right)$
...
En multipliant par $J_n(\beta)^{-1/2}$ à gauche, on a :
...
Comment peut-on exploiter ce résultat pour estimer l’écart-type de chaque coordonnée de $\hat{\beta}$ ? On donnera la démarche concrète à appliquer, mais on ne demande pas
de la mettre en pratique numériquement.
...
-   On calcule l'inverse de $J_n(\widehat{\beta})$ qui correspond à une
    estimation de la matrice de variance covariance de asymptotique de
    $\widehat{\beta}$.

-   L'élément diagonal numéro $j$ de cette dernière matrice correspond
    donc à une estimation de la variance asymptotique de la coordonnée
    $\widehat{\beta}_j$ . Sa racine carrée constitue ainsi une
    estimation de l'écart-type de $\widehat{\beta}_j$.
...
Cette procédure d’estimation est utilisée par la fonction `glm`. 
En utilisant sa sortie, donner une estimation de l’écart-type de 
$\hat{\beta}_0$ et $\hat{\beta}_1$.

::: {.content-visible when-meta="is_answer_print_15"}

::: answer
Il s'agit de la deuxième colonne dans la sortie de `summary`. Cela nous
donne un écart type estimé de 1,13365 pour $\widehat{\beta}_0$ et de
...
Construire un intervalle de confiance asymptotique à 95 % pour le paramètre $\beta_1$.
...
D’après la question précédente, le paramètre $\beta_1$ est-il différent de 0 
au seuil d’erreur asymptotique 5 % ?
Quel est le nom de cette procédure de test. 
Donner la $\textit{p-value}$ associée à ce test et vérifier qu’elle concorde bien avec la sortie de `glm`.

::: {.content-visible when-meta="is_answer_print_17"}

::: answer

Comme $0$ n'appartient pas à l'intervalle de confiance précédent,
$\beta_1$ est différent de $0$ au seuil d'erreur asymptotique 5 %.

Le test qui consiste à regarder si un paramètre est nul en s'appuyant
sur la normalité asymptotique de son estimateur est connue sous le nom
de "test de Wald".
...
Numériquement on obtient $\textit{p-valeur} = 4,022426\times10^{-06}$.
C'est la même valeur que dans la sortie de la fonction `glm`.
...
Calculer la statistique du test de déviance de significativité du modèle GLM (par
rapport au modèle nul). En déduire la p-value et conclure aux seuils d’erreur 10 %,
5 % et 1 %. Comparer avec les résultats du test effectué sous {{< iconify logos:r-lang >}} à l’aide de la fonction
`anova` appliqué au modèle, avec l’option `test="Chisq"`.
...
répartition de la loi $\mathcal{\chi}^2(d)$ en $x$ est calculable sous {{< iconify logos:r-lang >}}
...
est calculable sous {{< iconify logos:r-lang >}} grâce à la fonction `qchisq` via l'expression
...
Cette statistique s'exprime en fonction des déviances du modèle nul
$D_{H_0}$ et la déviance du modèle estimé $D_{H_1}$. En effet, la
défiance $D$ d'un modèle (logistque ou autre) vaut :
...
La p-valeur vaut $1-F_{\mathcal{N}(0,1)}(T)$ où $T$ est la statistique
de test.
...
Reprendre le graphique effectué à la question 7 et superposer (sous forme d’une
courbe) les valeurs prédites $\hat{p}(x)$ par le modèle logistique, calculées pour une grille de valeurs de $x$ couvrant l’étendue prise par les observations. On pourra utiliser la fonction `predict` associée à l’option `type="response"`.
...
En partant de la convergence en loi de $\hat{\beta}$ rappelée ci-dessus, en déduire un intervalle de confiance au niveau asymptotique 95 % pour $p(x)$.
Ajouter cet intervalle de confiance pour chaque $x$ considéré au graphique précédent.
On pourra exploiter avec profit l’option `se=TRUE` de la fonction `predict` dans le cas `type="link"`.
