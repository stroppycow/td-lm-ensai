...
C'est ce qu'on doit faire en pratique. Les arguments sont semblables à la fonction `lm`. Il faut faire attention à bien paramétrer l'argument `family` sans quoi on estimerait un modèle de régression linéaire simple.
...
On doit prendre ici `family=binomial()` car $Y$ sachant $X$ suit une loi de Bernoulli. 
...
On peut lire l'estimation par maximum de vraissemblance : $\widehat{\beta_0} = -5,30945$ et
...
C'est la même valeur que dans la sortie GLM.
