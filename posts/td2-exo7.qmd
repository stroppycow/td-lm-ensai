...
Le jeu de données `ozone.txt` contient la concentration maximale d’ozone ($\texttt{maxO3}$)
mesurée chaque jour de l’été 2001 à Rennes. Il contient également les températures, la nébulosité et la vitesse du vent mesurés à 9h, 12h et 15h (respectivement $\texttt{T9}$, $\texttt{T12}$, $\texttt{T15}$, $\texttt{Ne9}$, $\texttt{Ne12}$, $\texttt{Ne15}$ et $\texttt{Vx9}$, $\texttt{Vx12}$, $\texttt{Vx15}$), ainsi que la direction principale du vent et la présence ou non de pluie. On désire expliquer au mieux la concentration d’ozone à l’aide des variables disponibles dans le jeu de données.

## Question 1

Analyser le nuage de points et la corrélation linéaire entre $\texttt{maxO3}$ et chacune des variables quantitatives disponibles (c’est à dire $\texttt{T9}$, $\texttt{T12}$, $\texttt{T15}$, $\texttt{Ne9}$, $\texttt{Ne12}$, $\texttt{Ne15}$, $\texttt{Vx9}$, $\texttt{Vx12}$ et $\texttt{Vx15}$). Est-il raisonnable de supposer qu’il existe un lien linéaire entre $\texttt{maxO3}$ et ces variables ?
...
(exprimée en mètre), $\texttt{T9}$, $\texttt{T12}$, $\texttt{T15}$ pour les températures, $\texttt{Ne9}$, $\texttt{Ne12}$, $\texttt{Ne15}$ pour les nébulosités et $\texttt{Vx9}$, $\texttt{Vx12}$ et $\texttt{Vx15}$ pour la
vitesse du vent.

On représente ces données dans des plans ou l'axe des ordonnées est
toujours $\texttt{maxO3}$ : la variable à expliquer.
...
On observe une relation linéaire croissante assez prononcée avec $\texttt{T9}$, $\texttt{T12}$ et $\texttt{T15}$, une légère relation décroissante avec $\texttt{Ne9}$, $\texttt{Ne12}$ et $\texttt{Ne15}$, ainsi qu'une faible relation linéaire avec $\texttt{Vx9}$, $\texttt{Vx12}$ et $\texttt{Vx15}$. Aucun lien non-linéaire ne semble ressortir de manière évidente.
...
Ajuster le modèle de régression linéaire expliquant $\texttt{maxO3}$ en fonction de toutes les
variables quantitatives précédentes. Tester la significativité de chacune des variables
explicatives dans ce modèle. Le résultat est-il en accord avec les observations de la
question précédente ? Tester également la significativité globale du modèle. Commenter.
...
l'exception peut-être de $\texttt{Vx9}$ au seuil $\alpha =0,05$. Ces résultats
sont assez surprenants, surtout compte tenu des nuages de points
observés précédemment. En particulier, $\texttt{Vx9}$` ne semblait pas être la
...
rejette l'hypothèse nulle $H_0: \ \beta_2 = \dots =  \beta_{10}=0$. Cette
...
du modèle précédent. En quoi ces valeurs expliquent les résultats des tests de Student
effectués ci-dessus ?
...
On décide d’enlever des variables au modèle précédent. Quelles variables semblet-il naturel d’enlever au vu de la question précédente ? Ajuster le nouveau modèle proposé et répéter les analyses effectuées dans les deux questions précédentes.
...
qui est le plus correlée avec la variable $\texttt{maxO3}$ à chaque fois.
...
sont significatives, à l'exception de $\texttt{Ne12}$, qui est à la limite. Il
...
approche. Nous n'avons pas perdu vraiment d'information $67\  \%$ de la variance reste expliquée par le modèle.
...
Mettre en oeuvre une sélection automatique du meilleur sous-modèle possible du
"gros" modèle ajusté dans la [question 2](#question-2), selon le critère BIC. On pourra utiliser la
fonction  `regsubsets` de la libraire `leaps` (puis `plot.regsubsets`) ou `step`. Comparer
le modèle retenu avec le modèle choisi à la question précédente.
...
donc le modèle avec les variables $\texttt{T12}$, $\texttt{Ne9}$ et $\texttt{Vx9}$ (ce choix est
...
Appliquer la sélection automatique précédente en vous basant sur d’autres critères
que BIC. Les modèles retenus sont-ils les mêmes ? Si non, lequel semble préférable ?
...
Analyser les résidus du modèle sélectionné à la question précédente par des représentations graphiques et en effectuant des tests d’homoscédasticité et de non-corrélation des résidus. Toutes les hypothèses d’un modèle linéaire semblent-elles vérifiées ?
...
s'implémente sous {{< iconify logos:r-lang >}} avec la fonction `bptest` de la librairie `lmtest`.
...
Par défaut sous {{< iconify logos:r-lang >}} : $z_i$ est simplement le vecteur des variables
...
corrélation des résidus. Ils s'implémentent sous {{< iconify logos:r-lang >}} avec la fonction
...
Afin de résoudre le problème d’auto-corrélation des résidus, on propose d’ajouter la
maximum d’ozone de la veille dans le modèle. Créer cette variable, que l’on nommera
$\texttt{maxO3v}$ et ajouter-la au jeu de données. Observe-t-on un lien linéaire entre $\texttt{maxO3}$
et $\texttt{maxO3v}$ ?
...
Ajuster le modèle de régression contenant $\texttt{maxO3v}$ comme variable explicative supplémentaire.
Analyser les résultats de l’ajustement : les hypothèses d’un modèle
linéaire sont-elles vérifiées ?

::: answer

On ajoute la covariable $\texttt{maxO3v}$ au modèle de la question 5.
...
Comparer ce dernier modèle au modèle sans $\texttt{maxO3v}$ à l’aide d’un test de Fisher et en comparant les différents critères de sélection (AIC, BIC, Cp de Mallows, R² ajusté).
