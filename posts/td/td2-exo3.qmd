...
On étudie la consommation de glaces aux Etats-Unis sur une période de 30
semaines du 18 Mars 1950 au 11 Juillet 1953. Les variables sont la
période (de la semaine 1 à la semaine 30), et en moyenne sur chaque
période : la consommation de glaces par personne ($\texttt{Consumption}$, en 1/2
litre), le prix des glaces ($\texttt{Price}$, en dollars), le salaire
hebdomadaire moyen par ménage ($\texttt{Income}$, en dollars), et la température
($\texttt{Temp}$, en degré Fahrenheit). Les données sont disponibles dans le
fichier `icecream-R.dat`.

## Question 1

Extraire les données et représenter la consommation en fonction des
...
On regarde le contenu du fichier `icecream-R.dat` avant de l’importer en {{< iconify logos:r-lang >}} :
...
#| edit: false
#| echo: false
readLines("data/icecream-R.dat", n = 5, encoding = "UTF-8") |>
  paste0(collapse = "\n") |>
  cat()
```


Il s’agit d’un fichier plat dont le séparateur est la tabulation. La première ligne contient un en-tête qui indique le nom des variables.

On charge alors les données du fichier `icecream-R.dat`  dans {{< iconify logos:r-lang >}} sous la forme d’un `data.frame`.
...
-   $\texttt{Period}$ pour la période (de la semaine 1 à la semaine 30)
...
-   $\texttt{Income}$ pour le salaire hebdomadaire moyen par ménage
-   $\texttt{Temp}$ pour la température moyenne en degré Fahrenheit
...
On propose de régresser linéairement la consommation sur les trois
variables $\texttt{Price}$, $\texttt{Income}$ et $\texttt{Temp}$, en supposant de plus qu’une
constante est présente dans le modèle. On note la constante $\beta_1$ et
les trois coefficients associés aux variables précédentes respectivement
$\beta_2$, $\beta_3$ et $\beta_4$. Réaliser la phase d’estimation de
cette régression et commenter le signe des coefficients estimés.

::: {.content-visible when-meta="is_answer_print_2"}

::: answer
On effectue la régression
$\texttt{Consumption}_i = \beta_1+\beta_2\texttt{Price}_i+\beta_3\texttt{Income}_i+\beta_4\texttt{Temp}_i+\epsilon_i$
...
estimations comme le propose l'énoncé. La consommation semble décroître en moyenne
...
Tester la significativité globale du modèle proposé, i.e.
$H_0 \ : \ \beta_2=\beta_3=\beta_4=0$ à l’aide du test de Fisher global.
...
Tester la significativité de la variable $\texttt{Price}$ dans ce modèle au seuil
de 5 %. Tester de même la significativité de $\texttt{Income}$, puis de $\texttt{Temp}$.
...
Comparer le modèle complet précédent et le modèle sans la variable
$\texttt{Price}$ à l’aide d’un test de Fisher :

1.  En basant le calcul sur la somme des carrés résiduelle de chaque
    modèle ;
2.  En basant le calcul sur le coefficient de détermination de chaque
    modèle ;
3.  En utilisant la fonction `linearHypothesis` de la librairie `car` ;
4.  En utilisant la fonction anova.

Quel est la différence entre ce test et le test de Student de
significativité de la variable $\texttt{Price}$ ?

::: {.content-visible when-meta="is_answer_print_5"}

::: answer
On estime un modèle sans la variable $\texttt{Price}$.
...
-   $SCR$ est la somme des carrés des résidus $\widehat{\epsilon}_i$
...
Comparer le modèle complet et le modèle sans la variable $\texttt{Price}$ et sans
la constante à l’aide d’un test de Fisher. Procéder selon les 4 manières
décrites ci-dessus.
...
::: answer
On estime un modèle sans la variable $\texttt{Price}$ et la constante. On
teste
...
On désire à présent prédire la consommation de glaces pour les données
suivantes : 

$$\texttt{Price} = 0.3, \quad \texttt{Income} = 85 \quad \textrm{et}  \quad \texttt{Temp} = 65$$
 
Proposer la prévision qui vous semble la meilleure au vu des modèles étudiés
précédemment. Donner un intervalle de prévision au niveau 95 % autour de
cette prévision.
...
Sous quelle hypothèse l’intervalle de prévision précédent est-il valable
? Vérifier-la en observant le QQ-plot des résidus de la régression et en
...
Shapiro-Wilk. L'hypothèse nulle de ce test est l'échantillon suit une loi normale.
...
Vérifier les autres hypothèses en rappelant la définition et en
calculant les VIF ("Variance Inflation Factor") de chaque variable
explicative et en effectuant une analyse graphique des résidus.
...
et $\texttt{Temp}$, et l’ajustement par le modèle linéaire, à l’aide de
