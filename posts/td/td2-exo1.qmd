...
mesures des hauteurs ($\texttt{ht}$) et des circonférences ($\texttt{circ}$) de 1 429
...
On regarde le contenu du fichier `eucalyptus.txt` avant de l’importer en {{< iconify logos:r-lang >}} :
...
#| edit: false
#| echo: false
readLines("data/eucalyptus.txt", n = 5, encoding = "UTF-8") |>
  paste0(collapse = "\n") |>
  cat()
```


Il s’agit d’un fichier plat dont le séparateur est l’espace et dans lequel les valeurs non numériques sont entourées de guillemets droits doubles (`"`). La première ligne contient un en-tête qui indique le nom des variables, à l’exception du numéro de ligne.

On charge alors les données du fichier `eucalyptus.txt` dans {{< iconify logos:r-lang >}} sous la forme d’un `data.frame`.
...
data_eucalyptus <- read.csv(
  file = "data/eucalyptus.txt", # à adapter selon l'emplacement du fichier de données
  header = TRUE,
  sep = " ",
  quote = '"'
)
...
$\texttt{ht}$ pour la hauteur (exprimée en mètre) et $\texttt{circ}$ pour la circonférence
des eucalyptus.
...
ggplot2::ggplot(
  ...
  mapping = ggplot2::aes(x = circ, y = ht)
) +
...
Une régression linéaire semble bien adaptée, les points étant grossièrement disposés le long d’une droite.
...
::: answer
On effectue la régression $y = \beta_1+\beta_2x+\epsilon$ à l'aide de
...
-   La première partie du tableau <tt>Residuals</tt> donne quelques
    ...
    régression linéaire. Pour rappel, le résidu $\widehat{\epsilon}_i$
    ...
    $$
    \widehat{\epsilon}_i = Y_i- \underbrace{(\widehat{\beta}_1 + \widehat{\beta}_2X_i)}_{\widehat{Y}_i}
    $$
...
-   La seconde partie notée <tt>Coefficients</tt> est présentée sous
    ...
    -   La première colonne (colonne <tt>Estimate</tt>) contient les
        ...
        L'estimation du coefficient $\widehat{\beta}_2$ s'interprète de
        la manière suivante : "Lorsque la circonférence d'un arbre
        augmente d'un centimètre, sa hauteur moyenne augmente d'environ
        0,26 mètre, soit 26 centimètres." Il s'agit ici d'une simple
        association/corrélation statistique et non d'un lien de causalité.
    ...
    -   La seconde colonne ( <tt>Std. Error</tt>) contient les
        ...
        $$
        \forall\ j \in \{1, \dots, p\}, \  \widehat{\sigma}_{\widehat{\beta}_j} = \widehat{\sigma}\sqrt{(X'X)^{-1}_{j,j}} \ \textrm{avec} \ \widehat{\sigma} = \sqrt{\frac{1}{n-p}\sum\limits_{i=1}^n \widehat{\epsilon}_i^2}
        $$
    ...
    -   Dans la troisième colonne (<tt>t. value</tt>) figure la valeur
        ...
        normalité, d'indépendance et d'homoscédasticité des erreurs,
...
-   La ligne <tt>Residual standard error</tt> correspond à l'estimation
    de l'écart-type $\sigma$ des erreurs de modélisation $\epsilon_i$
    . On lit donc $\widehat{\sigma} \approx 1,199$. Le nombre de degré
    de liberté inscrit sur la même ligne $n-p = 1~429-2 = 1~427$ car

    $$
    (n-p)\frac{\widehat{\sigma}^2}{\sigma^2} \sim \chi^2\left(n-p \right)
    $$
...
-   La ligne suivante donne la valeur du coefficient de détermination
    ...
    $$
    \begin{align*}
    R^2 &=\frac{\textrm{SCE}}{\textrm{SCT}}= \frac{\left\| \widehat{Y}-\overline{y}\mathbb{1}\right\|^2}{\left\| Y-\overline{y}\mathbb{1}\right\|^2} = \frac{\sum\limits_{i=1}^n \left(\widehat{y}_i-\overline{y_n} \right)^2}{\sum\limits_{i=1}^n \left(y_i-\overline{y_n} \right)^2} \\
    &= 1 - \frac{\textrm{SCR}}{\textrm{SCT}} = 1-\frac{\left\|\widehat{\epsilon} \right\|^2}{\left\|Y - \overline{y_n}\mathbb{1}\right\|^2}=1-\frac{\sum\limits_{i=1}^n \widehat{\epsilon}_i^2}{\sum\limits_{i=1}^n \left(y_i-\overline{y_n} \right)^2}
    \end{align*}
    $$
    ...
    $$R_a^2=1-\frac{n-1}{n-p}\frac{\textrm{SCR}}{\textrm{SCT}} = 1-\frac{n-1}{n-p}\frac{\left\|\widehat{\epsilon} \right\|^2}{\left\|Y - \overline{y_n}\mathbb{1}\right\|^2}=1-\frac{n-1}{n-p}\frac{\sum\limits_{i=1}^n \widehat{\epsilon}_i^2}{\sum\limits_{i=1}^n \left(y_i-\overline{y_n} \right)^2}$$
...
### Accès aux différentes valeurs calulées en R via `lm`


**Accès à** $\widehat{\beta}$
...
#| autorun: true
# Méthode 1
fit_lm_eucalyptus$coefficients

# Méthode 2
coefficients(fit_lm_eucalyptus)

# Méthode 3
summary(fit_lm_eucalyptus)$coefficients[,"Estimate"]
...
#| autorun: true
#Méthode 1
sigma(fit_lm_eucalyptus)

#Méthode 2
summary(fit_lm_eucalyptus)$sigma
```
...
**Accès à** $\widehat{\mathbb{V}}\left[\widehat{\beta} \right]$
...
#| autorun: true
# Méthode 1
vcov(fit_lm_eucalyptus)

# Méthode 2
summary(fit_lm_eucalyptus)$cov.unscaled*sigma(fit_lm_eucalyptus)**2
```
...
Retrouver les estimations fournies par {{< iconify logos:r-lang >}} en l’utilisant.
...
data_eucalyptus |>
  ...
  dplyr::select(beta_1_hat, beta_2_hat) |>
  t() |>
  matrix(ncol = 1)
```
:::
...
Calculer un intervalle de confiance à 95 % pour $\beta_1$ et $\beta_2$,
en supposant la normalité des données.
...
où $\hat{\sigma}_{\hat{\beta}_j}=\hat{\sigma} \sqrt{\left(X^{\prime} X\right)_{j j}^{-1}}$
...
confiance précédents restent-ils valables ?

::: {.content-visible when-meta="is_answer_print_5"}

::: answer
Pour que ces intervalles de confiance soient exacts (au sens où le
niveau de confiance est respecté pour tout $n$), l'erreur doit être
gaussienne. Cependant, ils deviennent valables asymptotiquement sous
des conditions de régularité assez faibles (variance finie du bruit,
notamment), grâce au théorème central limite qui assure que
$\widehat{\beta}$ suit approximativement une loi gaussienne quand $n$
est grand (cf. remarque slide 70 du cours). Comme ici
$n = 1\ 429 \gg 30$, on peut donc se fier à ces intervalles de
confiance même si la normalité des erreurs n'est pas vérifiée.
:::
...
ggplot2::ggplot() +
 ...
 ggplot2::geom_ribbon(
   data = confint_val,
   mapping = ggplot2::aes(ymin = lwr, ymax = upr, x = circ),
   fill = "red",
   alpha = 0.5
 ) +
 ggplot2::geom_abline(
...
On veut à présent prédire la hauteur d’une nouvelle série d’eucalyptus
de circonférences 50, 100, 150 et 200. Donner les estimateurs de la
taille de chacun d’entre eux et les intervalles de prévision à 95 %
associés, en supposant la normalité des données.
...
newdata$pm <- q975*sigma_hat*sqrt(1+1/n+(newdata$circ-x_bar)**2/((n-1)*var_x))
...
beta_1_hat <- fit_lm_eucalyptus$coefficients[1] 
beta_2_hat <- fit_lm_eucalyptus$coefficients[2]
x_bar <- data_eucalyptus |> dplyr::pull(circ) |> mean()
var_x <- data_eucalyptus |> dplyr::pull(circ) |> var()
q975 <- qt(p = 0.975, df = n-p)
sigma_hat <- sigma(fit_lm_eucalyptus)
...
newdata <- tibble::tibble(circ = c(50, 100, 150, 200)) |>
  dplyr::mutate(
    ...
    pm = q975*sigma_hat*sqrt(1+1/n+(circ-x_bar)**2/((n-1)*var_x)),
...
-   l’erreur due à $\epsilon_{n+1}$: elle est stable quelle que soit
    la valeur de $\texttt{circ}$ et incompressible car
    $\mathbb{V}\left[\epsilon_{n+1}\right]=\sigma^2$.
:::
...
Si le bruit $\epsilon$ ne suit pas une loi normale, les intervalles de
prévision précédents restent-ils valables ?
...
-   l’erreur due à $\epsilon_{n+1}$ : cette erreur suit sa propre loi
