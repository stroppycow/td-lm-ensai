...
::: answer
La fonction $S$ est une fonction polynomiale de degré 2. Elle est donc dérivable
sur $\mathbb{R}$. On note $S'$ la fonction dérivée de $S$. Pour
$m \in \mathbb{R}$, on a : 

$$S'(m)=\sum\limits_{i=1}^n 2(m-z_i)$$ 

Comme le coefficient associé au degré 2 de la fonction polynomiale $S$ vaut $n$ qui est strictement positif,
le minimum est atteint lorsque $S'(m) = 0$.
...
::: answer
Dans le cas où on ne dispose pas d'informations auxiliaires, on peut
...
$\beta$ qui minimise la quantité (cf. slide 56)
$$
