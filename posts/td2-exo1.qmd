...
On veut expliquer la hauteur des eucalyptus en fonction de leur circonférence à partir d’une régression linéaire simple. On dispose des mesures des hauteurs (`ht`) et des circonférences (`circ`) de 1 429 eucalyptus, qui se trouvent dans le fichier `eucalyptus.txt`.
...
Effectuer la régression $y=\beta_1+\beta_2x+\epsilon$ où $y$ représente la hauteur et $x$ la circonférence. Commenter les résultats.
...
Quelle est la formule théorique de $\hat{\beta}_1$ et $\hat{\beta}_2$ ? Retrouver les estimations fournies par R en l’utilisant.
...
Tracer l’estimateur de la droite de régression et un intervalle de confiance à 95 % de celle-ci. Que déduisez-vous de la qualité de l’estimation?
...
Ajouter à la représentation graphique de la question 6 les intervalles de prévision (associés aux mêmes valeurs de la circonférence).
...
Si le bruit $\epsilon$ ne suit pas une loi normale, les intervalles de prévision précédents restentils valables ?
